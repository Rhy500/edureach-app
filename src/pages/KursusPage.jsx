...
function CourseCard({ course, onNavigate }) {
  ...
  return (
    ...
    >
      <div className={`${subj?.light || "bg-gray-50"} h-32 flex items-center justify-center relative`}>
        <span className="text-4xl font-black" style={{ color: subj?.hex }}>{subj?.icon}</span>
        {course.isNew && (
        ...
        <span className="absolute bottom-3 left-3 bg-white/90 text-gray-600 text-[10px] font-bold px-2 py-0.5 rounded-full border border-gray-100">
          {course.level}
        </span>
      </div>
      <div className="p-4 flex flex-col flex-1">
        <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full ${subj?.color} inline-block w-fit mb-2`}>
          {subj?.label}
        </span>
        ...
        <div className="flex items-center gap-2 text-[11px] text-gray-400 mb-3">
          <span>{course.episodes} episode</span>
          <span>·</span>
        ...
        {hasProgress ? (
          <div>
            <div className="flex justify-between mb-1.5">
              ...
              <span className={`text-[10px] font-bold`} style={{ color: subj?.hex }}>{course.progress}%</span>
            </div>
            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
              <div className={`h-full rounded-full ${subj?.bar} transition-all`} style={{ width: `${course.progress}%` }} />
            </div>
          </div>
        ) : (
...
export default function KursusPage({ onNavigate }) {
  ...
  const filtered = useMemo(() => {
    let r = COURSES;
    if (activeSubject !== "semua") r = r.filter(c => c.subject === activeSubject);
    if (activeLevel !== "Semua") r = r.filter(c => c.level === activeLevel);
    ...
    if (sort === "episodes") r = [...r].sort((a, b) => b.episodes - a.episodes);
    if (sort === "new") r = [...r].sort((a, b) => (b.isNew ? 1 : 0) - (a.isNew ? 1 : 0));
    return r;
  }, [activeSubject, activeLevel, search, sort]);
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
            ...
            <div className="flex flex-wrap gap-2">
              {SUBJECTS.map(s => (
                <button
                  ...
                  className={`text-xs font-semibold px-4 py-2 rounded-xl border transition-all duration-150 active:scale-95
                    ${activeSubject === s.id
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm text-gray-400">
            <span className="font-bold text-gray-700">{filtered.length}</span> kursus ditemukan
          </p>
          {(activeSubject !== "semua" || activeLevel !== "Semua" || search) && (
            <button
              onClick={() => { setActiveSubject("semua"); setActiveLevel("Semua"); setSearch(""); }}
